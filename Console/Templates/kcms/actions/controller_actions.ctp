...
<?php else: ?>
    return $this->flash(__('Đã thêm dữ liệu thành công.'), 'default', array('class' => 'alert alert-success'), array('action' => 'index'));
<?php endif; ?>
}
}
<?php
...
<?php else: ?>
    return $this->flash(__('Dữ liệu đã được cập nhật thành công'), 'default', array('class' => 'alert alert-success'), array('action' => 'index'));
<?php endif; ?>
}
} else {
$options = array('conditions' => array('<?php echo $currentModelName; ?>.' . $this-><?php echo $currentModelName; ?>->primaryKey => $id));
...
<?php if ($wannaUseSession): ?>
    ...
    return $this->redirect(array('action' => 'index'));
<?php else: ?>
    return $this->flash(__('Dữ liệu đã được xóa thành công'), 'default', array('class' => 'alert alert-success'), array('action' => 'index'));
    } else {
    return $this->flash(__('Có lỗi trong quá trình xóa dữ liệu. Vui lòng kiểm tra và thử lại'), 'default', array('class' => 'alert alert-danger'), array('action' => 'index'));
    }
<?php endif; ?>
}
